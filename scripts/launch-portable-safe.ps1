...
function Show-ErrorMessage([string]$message) {
  ...
  [System.Windows.MessageBox]::Show($message, "Susi Calculator Launch Error") | Out-Null
}
...
function Ensure-StaticBuild {
  if (Test-Path (Join-Path $siteRoot "index.html")) {
    return $true
  }

  if (-not (Test-Path $npm)) {
    Show-ErrorMessage "The static build folder (.next-prod) is missing and Node.js is not installed on this PC."
    return $false
  }
  ...
  if (-not (Test-Path "node_modules")) {
    ...
    if ($LASTEXITCODE -ne 0) {
      Show-ErrorMessage "npm install failed."
      return $false
    }
  }
  ...
  if (-not (Test-Path "src\\data\\calculator-dataset.json")) {
    & $npm run excel:json
    if ($LASTEXITCODE -ne 0) {
      Show-ErrorMessage "npm run excel:json failed."
      return $false
    }
  }
  ...
  if ($LASTEXITCODE -ne 0 -or -not (Test-Path (Join-Path $siteRoot "index.html"))) {
    Show-ErrorMessage "Static build creation failed."
    return $false
  }
  ...
  try {
    while ($true) {
      ...
      try {
        ...
        $requestPath = "/"
        if ($requestLine -and $requestLine -match "^[A-Z]+\s+([^\s]+)") {
        ...
        Write-HttpResponse -stream $stream -statusCode 200 -statusText "OK" -contentType (Get-ContentType -path $filePath) -body $body
      } finally {
        if ($reader) {
          $reader.Dispose()
        }
...
if (-not (Ensure-StaticBuild)) {
  exit 1
}

$port = Get-FreePort -ports $portRange
if (-not $port) {
  Show-ErrorMessage "No free port was found between 3010 and 3030."
  exit 1
}
