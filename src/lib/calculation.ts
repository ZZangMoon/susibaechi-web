...
export function getComprehensivePlacementRows(
  ...
  const rows = Array.from(
    new Map(
      [...dataset.comprehensivePlacementTable, ...rebuiltRows].map((row) => [row.key, row] as const),
    ).values(),
  );
...
export function getSchoolRecordPlacementRows(
  ...
  const rows = Array.from(
    new Map(
      [...dataset.schoolRecordPlacementTable, ...rebuiltRows].map((row) => [row.key, row] as const),
    ).values(),
  );
