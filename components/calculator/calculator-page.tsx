...
import { ResultsTableV2 } from "@/components/calculator/results-table-v2";
...
export function CalculatorPage({ dataset, deployMeta }: CalculatorPageProps) {
  ...
  const calculatedRows = useMemo(() => calculateResults(runtimeDataset, input), [runtimeDataset, input]);
  ...
  const filteredRows = useMemo(() => {
    const next = calculatedRows.filter((row) => {
    ...
    return next;
  }, [calculatedRows, deferredRecruitmentUnitQuery, deferredUniversityQuery, filters]);
    ...
    <main className="mx-auto min-h-screen max-w-[1800px] px-6 py-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        <section className="space-y-5">
          ...
          <ResultsTableV2 rows={filteredRows} showRegion={filters.showRegionColumn} />
        </section>
      </div>
    </main>
  );
