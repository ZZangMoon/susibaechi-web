import type { ToolbarFilters } from "@/src/hooks/use-calculator-shared-state";
...
export function ResultsToolbarV2({
  filters,
  regionOptions,
  resultCount,
  onChange,
    ...
    <div className="space-y-4 rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-panel">
      ...
      <div className="grid gap-3 xl:grid-cols-5">
        ...
        <select
          ...
          onChange={(event) => onChange({ judgment: event.target.value as ToolbarFilters["judgment"] })}
          className="rounded-2xl border border-line bg-white px-4 py-3 text-sm outline-none transition focus:border-sky-500"
        >
      ...
      <div className="grid gap-3 xl:grid-cols-[1fr_180px_220px]">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-600">
          대학명, 모집단위명, 전형유형, 판정값으로 결과를 빠르게 좁혀볼 수 있습니다.
        </div>
        <button
          type="button"
          onClick={() => onChange({ showRegionColumn: !filters.showRegionColumn })}
          className="rounded-2xl border border-line bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-sky-500"
        >
          {filters.showRegionColumn ? "지역 열 숨기기" : "지역 열 보기"}
        </button>
        <select
