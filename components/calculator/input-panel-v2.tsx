...
export function InputPanelV2({
  ...
  const filteredSchoolOptions = useMemo(() => {
    const keyword = deferredSchoolQuery.trim().toLowerCase();
    if (!keyword) {
    ...
    return schoolOptions
      ...
      .slice(0, 12);
  }, [deferredSchoolQuery, schoolOptions]);

  function commitSchoolQuery(nextValue: string, school?: SchoolRecord) {
    onSchoolChange(nextValue, school);
  }
  ...
  return (
    <aside className="space-y-6 rounded-[28px] border border-white/80 bg-white/90 p-6 shadow-panel">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-sky-700">성적입력</p>
      ...
      <label className="block space-y-2">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">학교 검색</div>
        <div className="relative">
          <input
            ...
            onKeyDown={(event) => {
              ...
              const exactMatch = schoolOptions.find((school) => school.schoolName === trimmed);
              const nextSchool = exactMatch ?? filteredSchoolOptions[0];
              commitSchoolQuery(trimmed, nextSchool);
            ...
            onBlur={() => {
              window.setTimeout(() => {
                setIsSuggestionOpen(false);
                const trimmed = schoolQuery.trim();
                if (trimmed && trimmed !== input.schoolName.trim()) {
                  const exactMatch = schoolOptions.find((school) => school.schoolName === trimmed);
                  commitSchoolQuery(trimmed, exactMatch);
                }
              }, 120);
            }}
              ...
              {filteredSchoolOptions.map((school) => (
                <button
                  ...
                  onMouseDown={(event) => {
                    ...
                    setIsSuggestionOpen(false);
                    commitSchoolQuery(school.schoolName, school);
                  }}
        ...
        <p className="text-xs leading-5 text-slate-500">
          입력값은 학교명만 유지하고, 같은 이름 학교는 후보 목록에서 시도/행정구로 구분합니다.
        </p>
      </label>

      <section className="rounded-3xl border border-line bg-[#f8fafb] p-4">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">학교 정보</div>
      ...
      <section className="space-y-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">생기부 관련 입력</div>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            원본 입력 셀은 보존했습니다. 현재 확인된 계산기 결과표에서는 직접 참조가 드러나지 않아 상태값으로 유지합니다.
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">내신 성적</div>
          <p className="mt-1 text-xs leading-5 text-slate-500">
      ...
      <section className="space-y-4">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">모의고사 등급</div>
        <div className="grid gap-3 sm:grid-cols-2">
      ...
      <section className="rounded-3xl border border-line bg-[#fffdfa] p-4">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">시트 분석 요약</div>
        <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
          {workbookSummary.sheetRoles.slice(0, 4).map((sheet) => (
            <li key={sheet.name}>
              <span className="font-semibold text-slate-900">{sheet.name}</span> {sheet.role}
            </li>
          ))}
        </ul>
      </section>
